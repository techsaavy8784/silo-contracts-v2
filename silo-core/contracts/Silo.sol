...
contract Silo is Initializable, ISilo, ReentrancyGuardUpgradeable, LeverageReentrancyGuard {
    ...
    function liquidity() public view virtual returns (uint256 liquidity) {
    ...
    function convertToShares(uint256 _assets) external view virtual returns (uint256 shares) {
        ...
        (uint256 totalSiloAssets, uint256 totalShares) =
            SiloStdLib.getTotalAssetsAndTotalShares(configData, AssetType.Collateral);

        return SiloMathLib.convertToShares(_assets, totalSiloAssets, totalShares, MathUpgradeable.Rounding.Down);
    }
    ...
    function convertToAssets(uint256 _shares) external view virtual returns (uint256 assets) {
        ...
        return SiloMathLib.convertToAssets(_shares, totalSiloAssets, totalShares, MathUpgradeable.Rounding.Down);
    }
    ...
    function maxWithdraw(address _owner) external view virtual returns (uint256 maxAssets) {
        (maxAssets,) = SiloERC4626Lib.maxWithdraw(
            config, _owner, AssetType.Collateral, total[AssetType.Collateral].assets, liquidity()
        );
    }
    ...
    function previewWithdraw(uint256 _assets) external view virtual returns (uint256 shares) {
        ...
        return SiloMathLib.convertToShares(_assets, totalSiloAssets, totalShares, MathUpgradeable.Rounding.Up);
    }
    ...
    function convertToShares(uint256 _assets, AssetType _assetType) external view virtual returns (uint256 shares) {
        ...
        (uint256 totalSiloAssets, uint256 totalShares) =
            SiloStdLib.getTotalAssetsAndTotalShares(configData, _assetType);
        ...
        returns (uint256 assets)
    {
        ...
        (AssetType depositType, address shareTokenFrom, address shareTokenTo, uint256 liquidity) = 
            ...
            : (AssetType.Protected, configData.collateralShareToken, configData.protectedShareToken, liquidity());
    ...
    function previewBorrowShares(uint256 _shares) external view virtual returns (uint256 assets) {
        ...
        return SiloMathLib.convertToAssets(_shares, totalSiloAssets, totalShares, MathUpgradeable.Rounding.Up);
    }
    ...
    function maxRepay(address _borrower) external view virtual returns (uint256 assets) {
        ...
        return SiloMathLib.convertToAssets(shares, totalSiloAssets, totalShares, MathUpgradeable.Rounding.Up);
    }
        ...
        returns (uint256 assets, uint256 shares)
    {
        ...
        (assets, shares) =
            SiloERC4626Lib.deposit(_configData.token, msg.sender, _depositParams, total[_depositParams.assetType]);
        ...
        returns (uint256 assets, uint256 shares)
    {
        ...
        uint256 liquidity = _params.assetType == AssetType.Collateral ? liquidity() : type(uint256).max;

        (assets, shares) =
            SiloERC4626Lib.withdraw(collateralConfig.token, shareToken, _params, liquidity, total[_params.assetType]);
        ...
        returns (uint256 assets, uint256 shares)
    {
        ...
        (assets, shares) =
            SiloLendingLib.repay(configData, _assets, _shares, _borrower, msg.sender, total[AssetType.Debt]);
